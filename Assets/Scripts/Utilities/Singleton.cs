            ...
            lock (_lock) 
            {
                if (_instance == null) 
                {
                    ...
                    if (instances == null || instances.Length == 0) 
                    {
                        ...
                        Debug.Log("[Singleton]: An instance of `" + typeof(T) + "` is needed." + " So gameObject `" + instanceObj.name + "` was created" +
                    ...
                    else 
                    {
                        ...
                        {
                            Debug.LogWarning("[Singleton]: There is more than one instance of `" + typeof(T) + "` in your scene. Destroying all, keeping only one...");
